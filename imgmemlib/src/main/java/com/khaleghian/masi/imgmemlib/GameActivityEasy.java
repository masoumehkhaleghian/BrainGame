...
public class GameActivityEasy extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        SharedPreferences per=getSharedPreferences("Data",MODE_PRIVATE);
        record=per.getString("easyImgMem","0");
        //timer
        ...
        //lan
        String lan=LocaleHelper.getLanguage(GameActivityEasy.this);
        String sh_language=per.getString("language",lan);
        context= LocaleHelper.setLocale(GameActivityEasy.this, sh_language);
        resources = context.getResources();
        //end lan

        //FAB
        game=findViewById(R.id.floatingActionButtongame2);
        what=findViewById(R.id.floatingActionButtonwhat2);
        pause=findViewById(R.id.floatingActionButtonPause2);
                ...
                if (image1.equals(image2))
                {
                    ...
                    if (isolate.size()==3)
                    {
                        if (Integer.parseInt(record)<counterSelectMatchImg)
                        {
                            ...
                            SharedPreferences per=getSharedPreferences("Data",MODE_PRIVATE);
                            per.edit().putString("easyImgMem",record).apply();
                        }
                        ...
                        showDialog(resources.getString(R.string.part1win)+" "+CounterOfChoice+" "+resources.getString(R.string.part2win)+" 0:"+seconds+mes);
                    }
                    else
                    {
                        if (sound)
